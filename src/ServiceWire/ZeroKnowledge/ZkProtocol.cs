...
namespace ServiceWire.ZeroKnowledge
{
    ...
    public class ZkProtocol
    {
        ...
        private readonly ZkBigInteger _n;
        ...
        public ZkProtocol()
        {
            ...
            _n = new ZkBigInteger(ZkSafePrimes.N4);
        }
        ...
        /// <returns></returns>
        public ZkPasswordHash HashCredentials(string identity, string identityKey)
        {
        ...
        /// <returns></returns>
        public byte[] ClientComputeSessionKey(byte[] salt, string identity, string identityKey, byte[] aEphemeral, byte[] bEphemeral, byte[] uScramble)
        {
            var key = ComputeHash(salt, identity.ConvertToBytes(), identityKey.ConvertToBytes());
        ...
        /// <returns></returns>
        public byte[] ClientCreateSessionHash(string identity, byte[] salt, byte[] aEphemeral, byte[] bEphermeral, byte[] sessionKey)
        {
        ...
        public byte[] CryptRand(int bits = 4096)
        {
            ...
            var bigrand = new ZkBigInteger(rb);
            var crand = bigrand % ZkSafePrimes.GetSafePrime(_random.Next(0, 2047)) ^ _n;
            var bytes = ZkBigInteger.ToByteArray(crand);
